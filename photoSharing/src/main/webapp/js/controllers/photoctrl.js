...
photoApp.controller('photoController', function($location, $scope, $rootScope,
		$http, $routeParams, $window, $cookies, apiService, photoData) {
	...
	$('html').click(
			function(e) {
				...
				} else if ((cid = commentEditOpen())
						&& (e.target != $('#editText' + cid)[0])) {
					if (editCount > 0) {
	...
	$scope.shareEdit = function(user) {
		apiService.editPhoto($scope.photo.editurl, $scope.photo.id, '?pid='
				+ $scope.photo.pid + '&share=' + user, successCallback,
				errorCallback);
	...
	$scope.editTitle = function() {
		...
		apiService.editPhoto($scope.photo.editurl, $scope.photo.id, '?pid='
				+ $scope.photo.pid + '&title=' + $scope.photo.title,
	...
	$scope.editCaption = function() {
		...
		apiService.editPhoto($scope.photo.editurl, $scope.photo.id, '?pid='
				+ $scope.photo.pid + '&caption=' + $scope.photo.summary,
				successCallback, errorCallback);
	...
	$scope.updateVisibility = function(visibility) {
		...
		apiService.editPhoto($scope.photo.editurl, $scope.photo.id, '?pid='
				+ $scope.photo.pid + '&visibility=' + visibility,
				successCallback, errorCallback);
	...
	$scope.editComment = function(content, cid) {
		...
		apiService.editComment(content, '?uid=' + $scope.photo.uid + '&pid='
				+ $scope.photo.pid + '&cid=' + cid, editCallback,
				editErrorCallback);
	...
	$scope.editPhoto = function(content) {
		apiService.editPhoto($scope.photo.editurl, $scope.photo.id, '?pid='
				+ $scope.photo.pid + '&q=' + content, editPhotoCallback,
				errorCallback);
	...
	$scope.deleteComment = function(cid) {
		...
		comment.loading = true;
		apiService.deleteComment('?cid=' + cid + '&pid=' + $scope.photo.pid
				+ '&uid=' + $scope.photo.uid, deleteCallback, errorCallback);
	...
	$scope.deleteTag = function(tag) {
		apiService.deleteTag($scope.photo.editurl, $scope.photo.id,
				$scope.photo.pid, tag, deleteTagCallback, errorCallback);
	...
	$scope.addComment = function() {
		...
		scrollComments();
		$scope.content = '';
		apiService.addComment(comment.content, '?pid=' + $scope.photo.pid
				+ '&uid=' + $scope.uid, $scope.photo.commenturl,
				addCommentCallback, addCommentErrorCallback);
	...
	$scope.like = function() {
		...
		apiService.postLike(params).then(function(data, status) {
			if ($scope.liked) {
				$scope.liked = false;
				$scope.photo.likes -= 1;
			} else {
	...
	$scope.deletePhoto = function() {
		...
		if (r == true) {
			...
			apiService.deletePhoto(params).then(function(data, status) {
